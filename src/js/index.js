...
// Rota para obter o nome do usuário com base no e-mail
app.get("/usuario/nome", async (req, res) => {
    try {
        const emailUsuario = req.query.email;

        // Aqui você precisa fazer uma consulta ao banco de dados para obter o nome do usuário com base no e-mail
        // Supondo que você esteja usando pool de conexão com o banco de dados e que a consulta retorne um objeto { nomeUsuario }
        const resultado = await pool.query("SELECT nome FROM tbusuario WHERE mail = $1", [usuario]);
